...
- 支持 `ZXY_FORCE_INSTALL_XRAY=1` 强制重装 Xray。
- 支持 `ZXY_SKIP_XRAY_INSTALL=1` 跳过 Xray 安装。
- 默认使用 `DEBIAN_FRONTEND=noninteractive` 和 `NEEDRESTART_MODE=a`，减少安装卡住。
- 安装完成后自动执行 `zxy-panel doctor`。
...
该命令会检查 API、Agent、Xray、Nginx、面板端口反代、Xray 配置文件、系统升级清单和最近 3 分钟 Xray 重启情况，适合安装完成后快速判断服务是否正常。
