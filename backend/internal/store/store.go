...
func normalize(d *model.PanelData) {
	...
	d.Version = "0.7.5.8.1-diagnosis-polish-upgrade-fix-agent-xray"
}
...
func pickLocalServer(list []model.Server) model.Server {
	...
	for _, srv := range list[1:] {
		if srv.AgentVersion == "0.7.5.8.1-diagnosis-polish-upgrade-fix-agent-xray" && keep.AgentVersion != "0.7.5.8.1-diagnosis-polish-upgrade-fix-agent-xray" {
			keep = srv
			continue
		}
